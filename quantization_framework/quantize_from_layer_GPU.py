...
class BestRQBrain(sb.core.Brain):
    ...
    def compute_forward(self, batch):
        ...
        to_be_quantized = hidden_states_list[self.hparams.quantize_layer]
        tgts = hparams["LayerQuantizer"](to_be_quantized)
        return tgts
...
hparams["LayerQuantizer"] = hparams["LayerQuantizer"].to(brain.device)
...
if not os.path.exists(save_targets_folder):
    os.makedirs(save_targets_folder)
    print(f"Created directory: {save_targets_folder}")
else:
    print(f"Directory already exists: {save_targets_folder}")

# load quantizer
quantizer_params_location = f"{save_targets_folder}/LayerQuantizer.pth"
if os.path.exists(quantizer_params_location):
    print(f"Loading layer quantizer from {quantizer_params_location}")
    hparams["LayerQuantizer"].load_state_dict(torch.load(quantizer_params_location))
else:
...
with torch.inference_mode():
    ...
    print('in train')
    train_save_location = f"{save_targets_folder}/train/"
    ...
    # go through train data_set
    print('in valid')
    valid_save_location = f"{save_targets_folder}/valid/"
...
train_df.to_csv(f"{save_targets_folder}/train_tgts.csv", index=False)
...
valid_df.to_csv(f"{save_targets_folder}/valid_tgts.csv", index=False)
...
if os.path.exists(quantizer_params_location):
    ...
    torch.save(hparams["LayerQuantizer"].state_dict(), quantizer_params_location)
