...
lr=0.0004
output_folder=results/fyi_noaug
train_csv=store/tgts_fiy.csv
...
python -m torch.distributed.run --nproc_per_node=2 --rdzv_backend c10d --rdzv-endpoint=localhost:0 $train $hparams --find_unused_parameters \
    ...
    --train_csv $train_csv --valid_csv $valid_csv --debug
...
lr=0.0004
output_folder=results/fyi_aug_ns
train_csv=store/tgts_fiy.csv
valid_csv=store/tgts_valid.csv
...
lr=0.0004
output_folder=results/fyi_aug_nsm
train_csv=store/tgts_fiy.csv
valid_csv=store/tgts_valid.csv
